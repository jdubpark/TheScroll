    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!(/^\d+$/.test(articleId))){
          ...
          const query =
            'SELECT t1.id, t1.author_display, t1.section, t1.time_published, t2.summary, t3.content, t4.link as image_link,'+
            ' t4.caption as image_caption, t5.link as video_link, t5.caption as video_caption FROM ArticleT1 t1'+
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!(/^\d+$/.test(limitCount)) || !(/^\d+$/.test(limitStart))){
          ...
          dbcon.query(query, [limitStart, limitCount], (err, articles, fields) => {
            if (err) reject(err);
